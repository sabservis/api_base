...
final class OpenApiGenerator
{
	...
	private function buildServers(): array|null
	{
		...
		foreach ($this->config->servers as $server) {
			...
			if (isset($server['variables']) && is_array($server['variables'])) {
				foreach ($server['variables'] as $name => $varConfig) {
					$variables[$name] = new ServerVariableObject(
						default: $varConfig['default'],
						description: $varConfig['description'] ?? null,
						enum: $varConfig['enum'] ?? [],
					);
				}
			}
	...
	private function buildOperation(Endpoint $endpoint, Schema $schema): OperationObject
	{
		...
		// Get summary, description and deprecated from Operation attribute
		$summary = $endpoint->getSummary();
		$description = $endpoint->getDescription();
		$deprecated = $endpoint->isDeprecated() ? true : null;
		$security = $endpoint->getSecurity();
	...
	{
		// If schema has a class reference, register it and return $ref
		$schemaRef = $param->getSchemaRef();

		if ($schemaRef !== null && class_exists($schemaRef)) {
			return $this->schemaBuilder->getReference($schemaRef);
		}

		// Use explicit schema spec if provided (from #[QueryParameter(schema: ...)] etc.)
		$schemaSpec = $param->getSchemaSpec();

		if ($schemaSpec !== null) {
			return new SchemaObject(
				type: $schemaSpec['type'] ?? null,
				format: $schemaSpec['format'] ?? null,
				enum: $schemaSpec['enum'] ?? null,
				minimum: $schemaSpec['minimum'] ?? null,
				maximum: $schemaSpec['maximum'] ?? null,
				minLength: $schemaSpec['minLength'] ?? null,
				maxLength: $schemaSpec['maxLength'] ?? null,
				pattern: $schemaSpec['pattern'] ?? null,
				default: $schemaSpec['default'] ?? null,
				example: $schemaSpec['example'] ?? null,
				nullable: $schemaSpec['nullable'] ?? null,
			);
		}
	...
	private function buildSchemaFromSpec(array $spec): SchemaObject
	{
		...
		// Handle additionalProperties (can be bool or schema object)
		$additionalProperties = null;

		if (isset($spec['additionalProperties'])) {
			if (is_bool($spec['additionalProperties'])) {
				$additionalProperties = $spec['additionalProperties'];
			} elseif (is_array($spec['additionalProperties'])) {
				$additionalProperties = $this->buildSchemaFromSpec($spec['additionalProperties']);
			}
		}
		...
		return new SchemaObject(
			...
			example: $spec['example'] ?? null,
			nullable: $spec['nullable'] ?? null,
			deprecated: $spec['deprecated'] ?? null,
			readOnly: $spec['readOnly'] ?? null,
			writeOnly: $spec['writeOnly'] ?? null,
			ref: $ref,
			items: $items,
	...
	private function buildResponses(Endpoint $endpoint): array
	{
		...
		foreach ($endpoint->getResponses() as $response) {
			...
			if ($response->isFileResponse()) {
			...
			} elseif ($response->isEntityOneOf() && is_array($entity)) {
				// List response with oneOf (multiple types)
				$schemaNames = $this->registerMultipleClasses($entity);
				...
				if ($response->isListWithMeta()) {
					...
					// Non-list oneOf (rare but possible)
					$schema = $this->buildOneOfSchema($schemaNames);
				}

				$content = [
					'application/json' => new MediaTypeObject(schema: $schema),
				];
			} elseif ($entity !== null && is_string($entity) && class_exists($entity)) {
				$schemaName = $this->schemaBuilder->registerClass($entity);
		...
		// Add default error response if enabled and not already defined by user
		if ($this->config->includeDefaultErrorResponse && !isset($responses['default'])) {
			$errorSchemaName = $this->schemaBuilder->registerClass(ErrorResponse::class);
			$responses['default'] = new ResponseObject(
				description: 'Error response',
				content: [
					'application/json' => new MediaTypeObject(
						schema: new SchemaObject(ref: $errorSchemaName),
					),
				],
			);
		}
	 ...
	 * @param array<string> $schemaNames
	 */
	private function buildListWithMetaSchemaOneOf(array $schemaNames): SchemaObject
	{
		return new SchemaObject(
			type: 'object',
			properties: [
	...
	/**
	 * Build inline schema for list without meta wrapper using oneOf.
	 * Structure: array with oneOf items
	 *
	 * @param array<string> $schemaNames
	 */
	private function buildListNoMetaSchemaOneOf(array $schemaNames): SchemaObject
	{
		return new SchemaObject(
			type: 'array',
			items: $this->buildOneOfSchema($schemaNames),
	...
	/**
	 * Build oneOf schema from multiple schema names.
	 *
	 * @param array<string> $schemaNames
	 */
	private function buildOneOfSchema(array $schemaNames): SchemaObject
	{
		$oneOfSchemas = array_map(
			static fn (string $name) => new SchemaObject(ref: $name),
			$schemaNames,
		);

		return new SchemaObject(oneOf: $oneOfSchemas);
	}

	/**
	 * Register multiple classes and return their schema names.
	 *
	 * @param array<string> $classNames
	 * @return array<string>
	 */
	private function registerMultipleClasses(array $classNames): array
	{
		$schemaNames = [];

		foreach ($classNames as $className) {
			if (class_exists($className)) {
				$schemaNames[] = $this->schemaBuilder->registerClass($className);
			}
		}
