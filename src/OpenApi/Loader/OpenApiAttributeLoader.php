...
final class OpenApiAttributeLoader
{
	...
	private function parseController(string $controllerClass, ReflectionClass $reflectionClass): array
	{
		...
		if ($securityAttributes !== []) {
			$securityAttr = $securityAttributes[0]->newInstance();
			assert($securityAttr instanceof Security);
			$controllerSecurity = $securityAttr->getSecurity();
		}
	...
	): array|null
	{
		...
		$parsed = $this->parseOperationAttributes($operationAttributes);
	 ...
	 * @param array<RequestOperationAttribute> $operationAttributes
	 */
	private function parseOperationAttributes(array $operationAttributes): ParsedOperation
	{
	...
	private function parseOperation(Operation $operation, ParsedOperation $parsed): void
	{
		...
		// Security from Operation attribute (inline)
		if ($operation->security !== null) {
			$parsed->security = $operation->security;
		}
	}
	...
	/**
	 * Parse #[Security] attribute from method.
	 */
	private function parseSecurityAttribute(ReflectionMethod $method, ParsedOperation $parsed): void
	{
	...
	/**
	 * Resolve tags: method tags override controller tags.
	 *
	 * @param array<string, mixed> $methodTags
	 * @param array<string, mixed> $controllerTags
	 * @return array<string, mixed>
	 */
	private function resolveTags(array $methodTags, array $controllerTags): array
	{
		// If method has its own tags, use them; otherwise inherit from controller
		return $methodTags !== [] ? $methodTags : $controllerTags;
	}

	/**
	 * Resolve security: method security overrides controller security.
	 *
	 * @param array<array<string, array<string>>>|string|null $methodSecurity
	 * @param array<array<string, array<string>>>|null $controllerSecurity
	 * @return array<array<string, array<string>>>|null
	 */
	private function resolveSecurity(array|string|null $methodSecurity, array|null $controllerSecurity): array|null
	{
		// If method explicitly sets security (including empty array for public), use it
		if (is_array($methodSecurity)) {
			return $methodSecurity;
		}

		// Otherwise inherit from controller (when SECURITY_INHERIT or null)
