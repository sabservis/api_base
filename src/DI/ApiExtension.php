...
use Sabservis\Api\Mapping\JsonSerializer;
use Sabservis\Api\Mapping\Normalizer\SimpleNormalizerProvider;
...
final class ApiExtension extends Nette\DI\CompilerExtension
{
	...
	private array $normalizers
		= [
			ArrayTypeNormalizer::class,
			BackedEnumTypeNormalizer::class,
			BooleanTypeNormalizer::class,
			DateTimeNormalizer::class,
			FloatTypeNormalizer::class,
			IntegerTypeNormalizer::class,
			ObjectTypeNormalizer::class,
			StringTypeNormalizer::class,
			UnitEnumTypeNormalizer::class,
		];
		...
		return Nette\Schema\Expect::structure([
			...
			'normalizer' => Nette\Schema\Expect::structure([
				'class' => Nette\Schema\Expect::anyOf(
					Nette\Schema\Expect::string(),
					Nette\Schema\Expect::type(Nette\DI\Definitions\Statement::class),
				)->default(SimpleNormalizerProvider::class),
				'types' => Nette\Schema\Expect::arrayOf('string')->default($this->normalizers),
			]),
	...
	private function compileDefinedMiddlewares(): void
	{
		...
		foreach ($config->middlewares as $service) {
			...
			$middlewareDef = $builder
				->addDefinition($this->prefix('middleware' . $counter++))
				->setFactory($service);
	...
	private function registerCoreMappingServices(Nette\DI\ContainerBuilder $builder): void
	{
		...
		$normalizer = $builder->addDefinition($this->prefix('request.entity.normalizer'))
			->setType($config->normalizer->class);

		foreach ($config->normalizer->types as $_normalizer) {
			$normalizer->addSetup('addNormalizer', [$_normalizer]);
		}
		...
		$builder->addDefinition($this->prefix('request.entity.serializer'))
			...
			->addSetup('addNormalizer', [$normalizer]);
