...
class PathValidation implements Validation
{
			...
			foreach ($controller->getMethods() as $method) {
				...
				foreach ($matches as $item) {
					...
					if ($match !== null) {
						throw (new InvalidSchemaException(
							sprintf(
								'@Path "%s" in "%s::%s()" contains illegal characters "%s" in parameter.
                                Allowed characters in parameter are only {[a-z-A-Z0-9-_]+}',
								$path,
								$controller->getClass(),
								$method->getName(),
								$match[1],
